window.addEventListener('DOMContentLoaded', () => {
  ...
  function setError(input, message) {
    ...
    inputBox.className = 'form__box form-box error';
  }

  const setSuccess = input => {
    const inputBox = input.parentElement;

    inputBox.className = 'form__box form-box success';
  ...
  const setEmpty = input => {
    ...
    inputBox.className = 'form__box form-box';
  }
  ...
  const sendForm = () => {
    form.addEventListener('submit', e => {
      ...
      if (isFormValid) {
        sendData('http://localhost:8080/test', JSON.stringify(data))
          .then(() => {
